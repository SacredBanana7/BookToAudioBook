# -*- coding: utf-8 -*-
"""
E2-TTS Voice-Cloning Test (Variante aus dem f5-tts Paket)
===========================================================
Unterschied zu test_f5tts.py:
- Nutzt E2-TTS statt F5-TTS (einfachere Architektur, manchmal besser bei Non-English)
- gen_text = leicht umgebaute Version der Whisper-Transkription des Referenz-Audios
  → phonetisch viel näher am Original, bessere Prosodieübertragung

Nutzung:
    python test_e2tts.py <stimme>
    python test_e2tts.py fred
"""
...
# torchaudio-Patch (torchcodec-DLL funktioniert nicht auf Windows/CUDA 12.8)
...
print(f"[whisper] Ref-Text original: {ref_text!r}")

# Zieltext = leicht umgebaute Version des Ref-Texts
# Ziel: ähnliche Phoneme/Rhythmus wie das Original, aber anderer Inhalt
def leicht_umbauen(text: str) -> str:
    ersetzungen = [
        ("jetzt",        "heute"),
        ("angefangen",   "begonnen"),
        ("muss",         "sollte"),
        ...
        ("schlafen",     "ausruhen"),
        ("selber",       "selbst"),
        ("vielleicht",   "vielleicht mal"),
    ]
    result = text
    for alt, neu in ersetzungen:
        result = result.replace(f" {alt} ", f" {neu} ", 1)
        result = result.replace(f" {alt},", f" {neu},", 1)
        result = result.replace(f" {alt}.", f" {neu}.", 1)
    return result

gen_text = leicht_umbauen(ref_text)
print(f"[whisper] Gen-Text (umgebaut): {gen_text!r}")

if not ref_text.strip():
    ref_text = "Hallo, das ist ein Testtext für das Sprachmodell."
    gen_text = "Hallo, das war ein kleiner Test für das neue Sprachmodell."
...
tts = F5TTS(model="E2TTS_Base", device=device)
print(f"[e2] Modell geladen in {time.time() - t0:.1f}s")
...
out_path = PROJECT_DIR / "test_output" / f"e2tts_{stimme}.wav"
