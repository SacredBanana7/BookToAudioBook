...
"""
Generiert "Der Code-Krieg" als VoiceBox-Story mit Fred und Flover (Chatterbox Engine).
Ausführen mit: python demos/story_fred_flover_chatterbox.py
"""
import sys, os
...
def main():
    print("=== Der Code-Krieg (Chatterbox) – VoiceBox Story ===\n")

    story = request("POST", "/stories", {"name": "Der Code-Krieg (Chatterbox)", "description": "Fred & Flover vs. die Bugs – Chatterbox Engine"})
    story_id = story["id"]
    ...
    print("Fuge Szenen zur Story hinzu...")
    for gen_id in gen_ids:
        request("POST", f"/stories/{story_id}/items", {"generation_id": gen_id})

    print(f"\nStory fertig! ID: {story_id}")
    print(f"Im VoiceBox Story-Editor unter 'Der Code-Krieg (Chatterbox)' oeffnen.")
